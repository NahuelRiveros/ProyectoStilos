...
export async function listarProductos(req, res) {
  try {
    ...
    if (categoria) {
      ...
      if (catPadre) {
        const catHijos = await Prod01Categoria.findAll({
          where:      { RELA_PARENT: catPadre.ID_PROD01, PROD01_FECHABAJA: null },
          attributes: ["ID_PROD01"],
        });
        const catIds = [catPadre.ID_PROD01, ...catHijos.map((c) => c.ID_PROD01)];
