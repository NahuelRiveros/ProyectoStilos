...
import type { Categoria } from "../../../api/catalogo_api";

// ─── Tipos exportados ─────────────────────────────────────────────────────────

export type Orden = "novedad" | "precio_asc" | "precio_desc" | "nombre_asc";
...
export function countActiveFiltros(f: Filtros): number {
  return [
...
const PRECIO_PRESETS: { label: string; value: number | null }[] = [
...
interface CatalogFiltersProps {
  ...
  urlCategoria?: string; // slug de la categoría que ya viene en la URL
}

// ─── Helper radio ─────────────────────────────────────────────────────────────

function RadioBtn({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={[
...
export default function CatalogFilters({
  ...
  function selectCategoria(slug: string | null) {
    set("categoriaSlug", filtros.categoriaSlug === slug ? null : slug);
  }

  // ── Árbol de categorías ───────────────────────────────────────────────────
  // Si la URL ya trae una categoría padre, mostrar solo sus hijas como sub-filtro.
  // Si no, mostrar raíces con hijas expandibles.
  const urlCatObj   = urlCategoria ? (categorias.find((c) => c.slug === urlCategoria) ?? null) : null;
  const childrenOf  = (id: number) => categorias.filter((c) => c.padre_id === id);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2.5">
          {([
            ["solo_ofertas", "Solo en oferta"] as const,
            ["solo_stock",   "Solo con stock"] as const,
          ]).map(([key, label]) => (
