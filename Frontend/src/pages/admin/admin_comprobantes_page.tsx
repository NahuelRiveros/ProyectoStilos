...
import { getTodasComprobantes, emitirComprobante, type Comprobante } from "../../api/comprobante_api";
import { getTiposComprobante, type TipoComprobante } from "../../api/catalogo_api";
import {
...
function EmitirModal({ tipos, onSave, onClose }: {
  ...
  onSave:  (ordenId: number, tipoId: number, pvId: number, cuit?: string) => Promise<void>;
  onClose: () => void;
}) {
...
export default function AdminComprobantesPage() {
  ...
  async function handleEmitir(ordenId: number, tipoId: number, pvId: number, cuit?: string) {
