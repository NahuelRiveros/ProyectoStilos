...
import ImageUploader, { type ImagenItem } from "../../components/ui/image_uploader";
import { ColoresSection, StockSection, type StockEdit } from "../../components/admin/product_form";
...
interface ProductForm {
  RELA_PROD01:            string;
  RELA_PROD02:            string;
  RELA_PROD07:            string;   // marca_id
  PROD03_NOMBRE:          string;
  PROD03_DESCRIPCION:     string;
  PROD03_PRECIO:          string;
  PROD03_PRECIO_ANTERIOR: string;
  PROD03_DESCUENTO:       string;
  PROD03_BADGE:           "" | "nuevo" | "vuelve" | "agotado";
  PROD03_HOME_SECCION:    "" | "carousel" | "novedades";
  PROD03_IMAGENES:        ImagenItem[];
...
function productoToForm(p: Producto): ProductForm {
  return {
    ...
    PROD03_BADGE:           (p.badge ?? "") as ProductForm["PROD03_BADGE"],
    PROD03_HOME_SECCION:    (p.home_seccion ?? "") as ProductForm["PROD03_HOME_SECCION"],
    PROD03_IMAGENES:        p.imagenes.map((img) => {
      if (typeof img === "string") return { src: img, alt: "", public_id: "" };
      const i = img as { src: string; alt?: string };
      return { src: i.src, alt: i.alt ?? "", public_id: "" };
    }),
    PROD03_COLORES: p.colores ?? [],
  };
}

function stockToEdit(rows: StockRow[], talles: Talle[]): StockEdit[] {
  if (rows.length > 0) {
    return rows.map((r) => ({
      talle_id: r.talle_id,
...
export default function AdminProductFormPage() {
  ...
  const [stock,      setStock]      = useState<StockEdit[]>([]);
  const [categorias, setCategorias] = useState<Categoria[]>([]);
  const [generos,    setGeneros]    = useState<Genero[]>([]);
  const [marcas,     setMarcas]     = useState<Marca[]>([]);
  const [tallesCat,  setTallesCat]  = useState<Talle[]>([]);
  const [coloresCat, setColoresCat] = useState<Color[]>([]);
  const [loading,    setLoading]    = useState(true);
  const [saving,     setSaving]     = useState(false);
  const [errors,     setErrors]     = useState<Partial<Record<keyof ProductForm, string>>>({});
  const [tab,        setTab]        = useState<"info" | "imagenes" | "stock" | "colores">("info");
  ...
  useEffect(() => {
    ...
    async function init() {
      // Cargar catálogos y producto en paralelo
      ...
      // Actualizar catálogos en estado
      setCategorias(cats);
      setGeneros(gens);
      ...
      if (productoData) {
        ...
        // Usamos `talles` directo (ya resuelto), no el estado que puede estar vacío
  ...
  function setStockField(i: number, field: "talle_id" | "cantidad", v: string) {
    setStock((p) =>
      p.map((row, idx) => {
        if (idx !== i) return row;
        if (field === "talle_id") {
  ...
  function handlePrecioBaseChange(v: string) {
    setPrecioBase(v);
    const base = Number(v);
  ...
  function handleDiscountPctChange(v: string) {
    setDiscountPct(v);
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-6 flex items-center gap-4">
        <button
          onClick={() => navigate(-1)}
          className="flex h-9 w-9 items-center justify-center rounded-xl border border-line text-muted hover:border-navy/40 hover:text-navy"
        >
          <ArrowLeft size={16} />
        </button>
        <div>
          ...
          <p className="text-sm text-muted">
            {isEditing ? `Editando ID #${id}` : "Completá los datos para publicar el artículo"}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* Tabs */}
            <div className="flex gap-0.5 rounded-xl border border-line bg-surface p-1">
              {([
              ...
              ] as const).map(({ id: tabId, label, icon: Icon, hasError }) => (
                <button
                  key={tabId}
                  type="button"
              ...
              <div className="space-y-5">
                <div className="rounded-2xl bg-card p-6 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="label-form">Nombre del artículo *</label>
                      <input
                      ...
                      />
                      {errors.PROD03_NOMBRE && <p className="error-form">{errors.PROD03_NOMBRE}</p>}
                    </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-3">
                      <div>
                        <label className="label-form">Tipo de prenda *</label>
                        <select
                        ...
                        {errors.RELA_PROD01 && <p className="error-form">{errors.RELA_PROD01}</p>}
                      </div>
                      <div>
                        ...
                        {errors.RELA_PROD02 && <p className="error-form">{errors.RELA_PROD02}</p>}
                      </div>
                      <div>
                ...
                <div className="rounded-2xl bg-card p-6 shadow-sm">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      <label className="label-form">Precio original *</label>
                      <div className="flex overflow-hidden rounded-xl border border-line bg-surface transition focus-within:border-navy focus-within:ring-1 focus-within:ring-navy/20">
                        <span className="flex shrink-0 items-center pl-3 pr-1 text-sm font-bold text-muted select-none">$</span>
                        <input
                      ...
                      {errors.PROD03_PRECIO && <p className="error-form">{errors.PROD03_PRECIO}</p>}
                    </div>
                    ...
                    <div>
                      <label className="label-form">
                        Descuento
                        <span className="ml-1 font-normal text-muted/60">(opcional)</span>
                      </label>
                      <div className="flex overflow-hidden rounded-xl border border-line bg-surface transition focus-within:border-navy focus-within:ring-1 focus-within:ring-navy/20">
                        <input
                          ...
                          placeholder="0"
                        />
                        <span className="flex shrink-0 items-center pl-1 pr-3 text-sm font-bold text-muted select-none">%</span>
                      </div>
                  ...
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-navy/20 bg-navy/5 px-4 py-3">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-navy/60">
                        Precio de venta
                      </p>
                      ...
                      {Number(form.PROD03_PRECIO_ANTERIOR) > 0 && Number(discountPct) > 0 && (
                        <>
                          ...
                          <span className="rounded-md bg-champagne/30 px-2 py-0.5 text-xs font-black text-ink">
                            -{discountPct}% · Ahorrás $
                            {(Number(form.PROD03_PRECIO_ANTERIOR) - Number(form.PROD03_PRECIO)).toLocaleString("es-AR")}
                          </span>
                        </>
                      )}
                      {Number(form.PROD03_PRECIO) > 0 && (
                  ...
                  <div className="mt-4">
                    <label className="label-form">
                      Etiqueta de oferta
                      <span className="ml-1 font-normal text-muted/60">(badge sobre la imagen)</span>
                    </label>
                    <input
            ...
            {tab === "imagenes" && (
              <div className="rounded-2xl bg-card p-6 shadow-sm">
                ...
                <p className="mb-4 text-xs text-muted">
                  Se suben a Cloudinary automáticamente. Podés reordenarlas arrastrando.
                </p>
                <ImageUploader
                ...
                <div className="mb-4 flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-muted">
                      Dejá un talle en 0 para marcarlo como agotado.
                    </p>
                  </div>
                </div>
                <StockSection
            ...
            {tab === "colores" && (
              <div className="rounded-2xl bg-card p-6 shadow-sm">
                <h2 className="mb-2 text-[11px] font-black uppercase tracking-widest text-muted">
                  Colores disponibles
                </h2>
                <p className="mb-4 text-xs text-muted">
                  Seleccioná los colores en que está disponible este artículo.
                </p>
                <ColoresSection
                  catalogo={coloresCat}
                  selected={form.PROD03_COLORES}
                ...
                {form.PROD03_COLORES.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {form.PROD03_COLORES.map((c) => (
                      <span
                        key={c.id}
                        className="flex items-center gap-1.5 rounded-full border border-line bg-surface px-2.5 py-1 text-xs font-semibold text-ink"
                      >
                        <span
                          className="h-3 w-3 rounded-full border border-line/40"
          ...
          <div className="space-y-5">
              ...
              <div className="space-y-2">
                {BADGE_OPTS.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setField("PROD03_BADGE", opt.value as ProductForm["PROD03_BADGE"])}
                    className={[
            ...
            <div className="rounded-2xl bg-card p-5 shadow-sm">
              ...
              <p className="mb-3 text-[11px] text-muted">
                ¿En qué sección del home aparece?
              </p>
              <div className="space-y-2">
                {([
                  ...
                  >
                    ...
                    <div>
                      <p className={[
                        "text-xs font-bold",
                        form.PROD03_HOME_SECCION === opt.value ? "text-navy" : "text-ink",
                      ].join(" ")}>
                        {opt.label}
                      </p>
              ...
              <div className="rounded-2xl bg-card p-5 shadow-sm">
                ...
                {form.PROD03_IMAGENES.length > 6 && (
                  <p className="mt-1.5 text-center text-xs text-muted">
                    +{form.PROD03_IMAGENES.length - 6} más
                  </p>
                )}
              </div>
            )}
            ...
            {form.PROD03_COLORES.length > 0 && (
              <div className="rounded-2xl bg-card p-5 shadow-sm">
                <h2 className="mb-3 text-[11px] font-black uppercase tracking-widest text-muted">
                  Colores ({form.PROD03_COLORES.length})
                </h2>
                ...
                <div className="space-y-1.5">
                  {stock.map((r, i) => (
                    <div key={i} className="flex items-center justify-between text-sm">
                      ...
                      <span className={[
                        ...
                        Number(r.cantidad) === 0 ? "text-rose-500" : "text-emerald-600",
                      ].join(" ")}>
                        {r.cantidad || 0}
                      </span>
                    </div>
                  ...
                  <div className="mt-2 border-t border-line pt-2 flex justify-between text-xs font-bold">
                    ...
                    <span className="text-ink">
                      {stock.reduce((s, r) => s + (Number(r.cantidad) || 0), 0)} uds
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-2xl bg-card p-5 shadow-sm">
              ...
              <button
                type="submit"
                disabled={saving}
                className="w-full rounded-xl bg-navy py-3 text-sm font-black uppercase tracking-widest text-white hover:bg-navy/90 disabled:opacity-60 transition-all active:scale-[0.98]"
              >
