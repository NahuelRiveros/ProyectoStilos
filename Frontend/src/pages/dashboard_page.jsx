...
const COLOR_MAP = {
  ...
  slate:   { icon: "bg-surface text-muted",          ring: "hover:ring-line"           },
  amber:   { icon: "bg-warning-surface text-warning",ring: "hover:ring-warning-border/60" },
...
function ModuloCard({ icon: Icon, label, desc, to, color }) {
  ...
  return (
    <NavLink
      to={to}
      className={[
        "group flex items-center gap-4 rounded-2xl border border-line bg-card p-4 shadow-sm transition-all duration-200 ring-1 ring-transparent",
...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      {/* Header */}
      <div className="mb-8">
        <p className="text-sm font-medium text-muted">{saludo},</p>
        <h1 className="mt-0.5 text-2xl font-black tracking-tight text-ink">
          {nombre}
        </h1>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          {usuario?.roles?.map((r) => (
            <span key={r} className="inline-flex items-center gap-1 rounded-full bg-surface px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-muted">
              {r}
            </span>
          ))}
        </div>
