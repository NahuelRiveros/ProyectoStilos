...
export interface CarouselSlide {
  badge?: string;
  title: string;
  titleAccent?: string; // se muestra en una línea aparte, en itálica champagne
  subtitle?: string;
  description?: string;
  image?: string;
  imageAlt?: string;
...
export default function HomeCarousel({
  ...
  // Barra de progreso: reset → 100 en cada cambio de slide
  useEffect(() => {
    if (!autoPlay || total <= 1) return;
    setBarWidth(0);
  ...
  return (
    <div className={UI_CAROUSEL.shell}>
      ...
      <div className={UI_CAROUSEL.viewport}>
        ...
        >
          {safe.map((slide, si) => (
            <div key={si} className={UI_CAROUSEL.slide}>
              <div className={UI_CAROUSEL.grid}>

                {/* Columna texto */}
                <div className={UI_CAROUSEL.content}>
                  {slide.badge && (
                    <span className={UI_CAROUSEL.badge}>{slide.badge}</span>
                  )}

                  <h2 className={UI_CAROUSEL.title}>
                    {slide.title}
                    {slide.titleAccent && (
                      <span className={[UI_CAROUSEL.titleAccent, "block"].join(" ")}>
                        {slide.titleAccent}
                      </span>
                    )}
                  </h2>
                  ...
                  {slide.subtitle && (
                    <p className={UI_CAROUSEL.subtitle}>{slide.subtitle}</p>
                  )}
                  {slide.description && (
                    <p className={UI_CAROUSEL.description}>{slide.description}</p>
                  )}

                  {slide.tags && slide.tags.length > 0 && (
                    <div className={UI_CAROUSEL.tags}>
                      {slide.tags.map((tag) => (
                        <span key={tag} className={UI_CAROUSEL.tag}>
                          {tag}
                        </span>
                      ))}
                    </div>
      ...
      {/* ── Barra de progreso ── */}
      {autoPlay && total > 1 && (
        <div className={UI_CAROUSEL.progressTrack}>
          <div
            className={UI_CAROUSEL.progressBar}
            style={{
              width: `${barWidth}%`,
              transition:
                barWidth === 0 ? "none" : `width ${intervalMs}ms linear`,
            }}
          />
        </div>
      )}
      ...
      {/* ── Controles ── */}
      {total > 1 && (
        <div className={UI_CAROUSEL.controlsOuter}>
