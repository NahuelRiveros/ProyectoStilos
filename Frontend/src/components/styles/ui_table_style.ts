export const UI_TABLE = {
  ...
  header:
    "flex flex-col gap-3 border-b border-line bg-surface/80 p-4 md:flex-row md:items-center md:justify-between",
  titleBox: "min-w-0",
  title: "text-base font-black text-ink",
  subtitle: "mt-1 text-sm font-medium text-muted",
  searchWrap:
    "flex w-full items-center gap-2 rounded-2xl border border-line bg-card px-3 py-2 md:max-w-xs",
  searchInput:
  ...
  footer:
    "flex items-center justify-between border-t border-line bg-surface px-4 py-3 text-xs font-semibold text-muted",
  actionVariants: {
    view:   { variant: "info" as const,    className: "" },
    edit:   { variant: "warning" as const, className: "" },
    delete: { variant: "danger" as const,  className: "" },
  },
} as const;
