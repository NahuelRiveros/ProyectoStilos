  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted">
        La barra de beneficios debajo del carousel. Exactamente 4 ítems.
      </p>
        ...
        <div key={i} className="grid gap-3 rounded-xl border border-line bg-surface px-4 py-3 sm:grid-cols-3">
          ...
          <div>
            <div className="mb-1 flex items-center justify-between">
              ...
              <EmojiPicker onPick={(e) => setPerk(i, { title: (perk.title ?? "") + e })} />
            </div>
            <input
              value={perk.title ?? ""}
              onChange={(e) => setPerk(i, { title: e.target.value })}
              className="input-form h-8 py-1.5 text-xs"
          ...
          <div>
            <div className="mb-1 flex items-center justify-between">
              ...
              <EmojiPicker onPick={(e) => setPerk(i, { text: perk.text + e })} />
            </div>
            <input
              value={perk.text}
              onChange={(e) => setPerk(i, { text: e.target.value })}
              className="input-form h-8 py-1.5 text-xs"
              placeholder="En compras +$50.000"
            />
          </div>
        </div>
      ))}
