...
export function NovedadesEditor({
  ...
  useEffect(() => {
    getProductos({ orden: "novedad", limit: 60 })
  ...
  const selectedProducts = useMemo(
    () => selectedIds.map(id => productos.find(p => p.id === id)).filter(Boolean) as Producto[],
    [selectedIds, productos],
  );

  function toggle(producto: Producto) {
  ...
  function getImage(p: Producto): string {
    ...
    <div className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {selectedProducts.map((p, i) => (
              <div key={p.id} className="relative overflow-hidden rounded-xl border-2 border-navy bg-card">
                <div className="relative aspect-[3/4]">
                  {getImage(p)
                    ? <img src={getImage(p)} alt={p.nombre} className="h-full w-full object-cover" />
      ...
      {selectedIds.length < 4 && (
        <div>
          ...
          {loading ? (
            <p className="py-4 text-center text-xs text-muted">Cargando productos…</p>
          ) : (
            <div className="max-h-72 overflow-y-auto rounded-xl border border-line divide-y divide-line">
              {filtered.filter(p => !selectedIds.includes(p.id)).slice(0, 30).map(p => (
              ...
              {filtered.filter(p => !selectedIds.includes(p.id)).length === 0 && (
      ...
      {selectedIds.length >= 4 && (
        <p className="rounded-xl border border-champagne/30 bg-champagne/10 px-4 py-2.5 text-xs font-semibold text-ink">
