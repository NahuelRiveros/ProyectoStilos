...
import { type AnnouncementItem, type AnnouncementIcon } from "../../../api/home_config_api";
import { ANNOUNCEMENT_ICONS } from "./home_config_helpers";

export function AnnouncementEditor({
  items, onChange,
}: { items: AnnouncementItem[]; onChange: (i: AnnouncementItem[]) => void }) {
  function setItem(i: number, upd: Partial<AnnouncementItem>) {
    onChange(items.map((item, idx) => idx === i ? { ...item, ...upd } : item));
  }
  function remove(i: number) { onChange(items.filter((_, idx) => idx !== i)); }
  function add() { onChange([...items, { icon: "sparkles", text: "", accent: "" }]); }
    ...
    <div className="space-y-3">
      ...
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2 rounded-xl border border-line bg-surface px-3 py-2.5">
          <select value={item.icon} onChange={e => setItem(i, { icon: e.target.value as AnnouncementIcon | "" })}
          ...
          <input value={item.text} onChange={e => setItem(i, { text: e.target.value })}
            className="min-w-0 flex-1 bg-transparent text-sm text-ink placeholder:text-muted/50 focus:outline-none"
            placeholder="Texto del anuncio…" />
          <input value={item.accent} onChange={e => setItem(i, { accent: e.target.value })}
