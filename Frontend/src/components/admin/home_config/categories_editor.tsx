...
import { type CategoryConfig } from "../../../api/home_config_api";
...
export function CategoriesEditor({
  categories, onChange,
}: { categories: CategoryConfig[]; onChange: (c: CategoryConfig[]) => void }) {
  const [expandedIdx, setExpandedIdx] = useState<number | null>(0);

  function update(i: number, patch: Partial<CategoryConfig>) {
  ...
  return (
    <div className="space-y-3">
      ...
      {categories.map((cat, i) => {
        ...
        return (
          <div key={cat.slug + i} className="overflow-hidden rounded-2xl border border-line bg-card">
            <div className="flex items-center gap-2 px-4 py-3">
              ...
              <div className={["relative h-10 w-8 shrink-0 overflow-hidden rounded-lg",
                SLUG_BG_OPTIONS[i] ?? SLUG_BG_OPTIONS[0]].join(" ")}>
                {cat.image && (
              ...
              <button type="button" onClick={() => remove(i)} title="Eliminar categoría"
            ...
            {isOpen && (
              <div className="border-t border-line px-4 pb-5 pt-4 space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    <input value={cat.name}
                      ...
                      className="input-form" placeholder="Ej: Mujer" />
                  </div>
                  <div>
                    <label className="label-form">Texto del hover</label>
                    <input value={cat.caption}
                      onChange={e => update(i, { caption: e.target.value })}
                      className="input-form" placeholder="Ver colección" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input value={cat.to}
                    ...
                    className="input-form" placeholder="/damas" />
                ...
                <div>
                  ...
                  {cat.image ? (
                    <div className="relative mb-2 overflow-hidden rounded-xl" style={{ aspectRatio: "3/5", maxHeight: 160 }}>
                      <img src={cat.image} alt={cat.name} className="h-full w-full object-cover" />
      ...
      {categories.length < 5 ? (
        ...
        <p className="rounded-xl border border-champagne/30 bg-champagne/10 px-4 py-2.5 text-xs font-semibold text-ink text-center">
